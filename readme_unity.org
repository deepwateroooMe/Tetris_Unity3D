...
public interface IEnumerator { // 这里是从网上搜来的C#里的
    bool MoveNext();
    Object Current {
        get; 
    }
...
// 第一次理解的直觉是对的:枚举器,不是协程
    internal class Adaptor : IEnumerator<object>, CrossBindingAdaptorType {
        ...
        public object Current { // 获取枚举器迭代器当前元素
            get {
...
- 上面说的牛头不对马尾.这里是 *适配,为什么需要使用适配呢?*
- 是因为在运行时,这是两个不同的程序域,运行时ILRuntime热更新模块并不知道unity中的类型,所以需要在 Unity 工程中写适配器来让 ILRuntime 知道如何调用 Unity 代码，或当 Unity 的事件触发时让 ILRuntime 能够监听到。
...
- 下面补一下之前没有理解透彻的关于 *CLR绑定* (是对可能会有的大量GC Alloc回收等开销进行的性能优化,包括框架中用到的对于vector2 vector3等的处理): 在 Hotfix 工程中，假如需要调用 Unity 工程的方法，ILRuntime 会通过反射对目标方法进行调用，这个过程会有因为装箱和拆箱等操作产生的大量 GC Alloc 和额外开销。因此需要借助 CLR 绑定 功能，通过将需要的函数调用进行静态绑定，如此调用时就不会出现 GC Alloc 和额外开销。
- CLR 绑定本质上是基于 CLR 重定向实现的(CLR 方法重定向)
- 程序集域:
...
[[./pic/readme_unity_20221007_082239.png]]
*** ModuleBase.cs
     #+BEGIN_SRC csharp
