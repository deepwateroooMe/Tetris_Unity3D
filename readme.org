...
** 
   #+BEGIN_SRC csharp
   #+END_SRC 
** HotFixILRunTime : SingletonMono<HotFixILRunTime>, IHotFixMain: 我忘记了什么叫SingletonMono ?
   #+BEGIN_SRC csharp
...
** CrossBindingAdaptor : IType 跨域(程序集)绑定适配器 + CrossBindingAdaptorType interface 公用接口类(为什么要这个公用接口？)
    #+BEGIN_SRC csharp
