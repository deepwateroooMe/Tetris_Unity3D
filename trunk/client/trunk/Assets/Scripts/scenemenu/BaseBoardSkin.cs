...
namespace tetris3d {
    ...
    public class BaseBoardSkin : MonoBehaviour { // could use collider as well, but so far let's do this way
        ...
        void OnEnable() {
            ...
            GameController.changeBaseCubesSkin += onChallengeTetrominoLand;
            ...
            for (int i = 0; i < n; i++) {
                if (cubes[i].activeSelf) {
                    if (cubes[i].GetComponent<MinoType>().color != Model.baseCubes[i]) {
                        ...
                        cubes[i].gameObject.GetComponent<Renderer>().sharedMaterial = PoolManager.Instance.materials[Model.baseCubes[i]];
                    }
                }
            }
            Debug.Log(TAG + ": baseCubes colors after updateSkin"); 
            MathUtil.printBoard(Model.baseCubes);
        }
            ...
            foreach (Transform mino in GameController.nextTetromino.transform) {
                if (mino.CompareTag("mino")) {
                    ...
                    if (pos.y == 0) {
                        ...
                        Debug.Log(TAG + " PoolManager.Instance.materials[Model.prevSkin[i]].ToString(): " + PoolManager.Instance.materials[Model.prevSkin[i]].ToString());
        ...
        int getChallengedMaterialIdx(Material material) {
            int n = PoolManager.Instance.materials.Length;
            for (int i = 0; i < n; i++) {
                if (PoolManager.Instance.materials[i] == material)
                    return i;
                else continue;
            }
            return -1;
        ...
        void onUndoGame(UndoGameEventInfo undoInfo) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                Debug.Log(TAG + " Model.prevIdx[i]: " + Model.prevIdx[i]);
                cubes[Model.prevIdx[i]].gameObject.GetComponent<Renderer>().sharedMaterial = PoolManager.Instance.materials[Model.prevSkin[i]];
