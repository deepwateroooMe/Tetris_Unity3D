...
namespace tetris3d {
    ...
    public class Model { 
            ...
            foreach (Transform mino in GameController.nextTetromino.transform) {
                if (mino.CompareTag("mino")) {
                    ...
                    if (GetTransformAtGridPosition(pos) != null
					    && GetTransformAtGridPosition(pos).parent != GameController.nextTetromino.transform) {
                        return false;
                    }
                }
            }
        ...
        public static bool IsFullRowAt(int y) {
            ...
            for (int x = 0; x < gridXWidth; x++)
                for (int j = 0; j < gridZWidth; j++) 
                    if ( (!GameController.isChallengeMode && (grid[x, y, j] == null 
                                                              || (grid[x, y, j].parent == GameController.ghostTetromino.transform && grid[x, y, j].parent != GameController.nextTetromino.transform))) 
                         // if ( ((grid[x, y, j] == null && !GameController.isChallengeMode) || grid[x, y, j].parent != GameController.nextTetromino.transform)
                         || (GameController.isChallengeMode && grid[x, y, j] == null && gridOcc[x, y, j] == 0) )
                        return false;
            numberOfRowsThisTurn++;
            return true;
        }
        ...
        public static void cleanUpGameBroad() {
            ...
            if (GameController.nextTetromino != null && GameController.nextTetromino.CompareTag("currentActiveTetromino")) { // hang in the air
                Debug.Log(TAG + " (GameController.ghostTetromino != null): " + (GameController.ghostTetromino != null));  // always true
                if (GameController.ghostTetromino != null) {
                    GameController.recycleGhostTetromino();
                }
                GameController.recycleNextTetromino();
            }
            int x = 0, y = 0, z = 0;
                        ...
                        if (grid[i, j, k] != null) {
                            if (grid[i, j, k].parent != null && grid[i, j, k].parent.childCount == 4) {
                                ...
                                foreach (Transform transform in grid[i, j, k].parent) {
                                    ...
                                    PoolManager.Instance.ReturnToPool(transform.gameObject, typeTmp);
                                }
                            }
                        }
                    }
        ...
        public static string GetRandomTetromino() { // active Tetromino
            ...
            if (GameController.gameMode == 0 && gridWidth == 3)
        ...
        public static string GetGhostTetrominoType(GameObject gameObject) { // GameController.ghostTetromino
