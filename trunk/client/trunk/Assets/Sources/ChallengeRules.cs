...
namespace tetris3d {
    ...
    public static class ChallengeRules {
        ...
        public static bool isValidLandingPosition() {
            ...
            if (GameController.nextTetromino.transform.childCount == 1)
                isSolo = true;
            ...
            if (GloData.Instance.challengeLevel < 11 && !isColorExistOnContactableBoard(GameController.nextTetromino.GetComponent<TetrominoType>().color))
                return true;
        ...
        public static bool isThereSolutionOnBoard() { // 暴力 查找 合理解
            ...
            int x = 0, y = 0, z = 0;
            x = (int)Mathf.Round(GameController.nextTetromino.transform.position.x);
            y = (int)Mathf.Round(GameController.nextTetromino.transform.position.y);
            z = (int)Mathf.Round(GameController.nextTetromino.transform.position.z);
            bool isMatching = false;
                ...
                for (int j = 0; j < Model.gridHeight; j++) {
                    for (int k = 0; k < Model.gridZWidth; k++) {
                        ...
                        if (delta != Vector3.zero) {
                            GameController.nextTetromino.transform.position += delta; // 平移
                            ...
                            if (isTetrominoMatchingNeighbourAfterRotation()) // 12
                                return true;
                            GameController.nextTetromino.transform.position -= delta;
                        }
                    }
                }
            }
        ...
        public static bool isTetrominoMatchingNeighbourAfterRotation() { // 旋转
            ...
            for (int i = 0; i < rotations.Length; i++) { // todo: 这个最好对每个 tetromino 进行优化处理，而不是每个都检查12次
                matchingCnter = 0;
                GameController.nextTetromino.transform.Rotate(rotations[i]); // 旋转有大量浪费计算，还需要考虑平移: 但仍然没有包括所有的landing可能性
                if (!Model.CheckIsValidPosition()) {
                    GameController.nextTetromino.transform.Rotate(-rotations[i]);
                    continue;
                } else {
                    if (isTetrominoMatchingNeighbour()) return true; 
        ...
        public static bool isExistInHigherLayers(int color) { 
            ...
            for (int i = 0; i < Model.gridXWidth; ++i) {
                for (int k = 0; k < Model.gridZWidth; k++) {
                    ...
                    if (j > 0 && Model.gridClr[i, j, k] == color && Model.grid[i, j, k].parent.gameObject != GameController.nextTetromino) {
        ...
        public static bool isMatchingAnyNeighbour(int x, int y, int z) { // getting the values through a hard way, get from board faster ?
            ...
            if (x-1 >= 0 && Model.grid[x-1, y, z] != null
                && Model.grid[x-1, y, z].parent.gameObject != GameController.nextTetromino) {
                // if (x-1 >= 0 && Model.GetTransformAtGridPosition(new Vector3(x-1, y, z)) != null
                // && Model.GetTransformAtGridPosition(new Vector3(x-1, y, z)).parent.gameObject != GameController.nextTetromino) {
            ...
            if (z-1 >= 0 && Model.grid[x, y, z-1] != null
                && Model.grid[x, y, z-1].parent.gameObject != GameController.nextTetromino) {
            ...
            if (y-1 >= 0 && Model.grid[x, y-1, z] != null
                && Model.grid[x, y-1, z].parent.gameObject != GameController.nextTetromino) {
        ...
        public static bool isNeighboursExist(int x, int y, int z) {
            ...
            if (z-1 >= 0 && Model.GetTransformAtGridPosition(new Vector3(x, y, z-1)) != null
                && Model.GetTransformAtGridPosition(new Vector3(x, y, z-1)).parent.gameObject != GameController.nextTetromino)
                return true;
            ...
            if (y-1 >= 0 && Model.GetTransformAtGridPosition(new Vector3(x, y-1, z)) != null
                && Model.GetTransformAtGridPosition(new Vector3(x, y-1, z)).parent.gameObject != GameController.nextTetromino)
                return true;
            return false;
        }
