    ...
    public static class MessageDispatcher {
        ...
        public static void Init() {
            ...
            foreach (Type type in types) {
                IMHandler iMHandler = Activator.CreateInstance(type) as IMHandler; // 生成处理器实例
                if (iMHandler == null) {
        ...
        public static void Handle(Session session, ushort opcode, object message) {
            if (!Handlers.TryGetValue(opcode, out var handler)) {
                Debug.LogError($"消息没有处理: {opcode} {message}"); // 消息没有处理: 10030 {"_t":"C2M_TransferMap","RpcId":262749}
                return;
            }
            try {
